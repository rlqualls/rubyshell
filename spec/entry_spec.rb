...
describe RubyShell::Entry do
  before do
    ...
    @entry = RubyShell::Entry.new(@filename)
  end
  ...
  it "knows its parent dir" do
    @entry.parent.should be_kind_of(RubyShell::Dir)
    @entry.parent.name.should == File.basename(@sandbox_dir)
    @entry.parent.full_path.should == @sandbox_dir
  end

  it "cleans its pathname" do
    RubyShell::Entry.new('/a//b//c').full_path.should == '/a/b/c'
    RubyShell::Entry.new('/1/2/../3').full_path.should == '/1/3'
  end
  ...
  it "considers itself equal to other instances with the same full path" do
    RubyShell::Entry.new('/not/the/same').should_not == @entry
    RubyShell::Entry.new(@entry.full_path).should == @entry
  end
  ...
  it "can't rename itself if another file already exists with that name" do
    ...
    lambda { @entry.rename(new_file) }.should raise_error(RubyShell::NameAlreadyExists, /#{new_file}/)
  end

  it "can't rename itself to something with a slash in it" do
    lambda { @entry.rename('has/slash') }.should raise_error(RubyShell::NameCannotContainSlash, /slash/)
  end

  it "can duplicate itself within the directory" do
    @entry.duplicate('newfile').should == RubyShell::File.new("#{@sandbox_dir}/newfile")
  end
  ...
  it "considers dotfiles to be hidden" do
    RubyShell::Entry.new("#{@sandbox_dir}/show").should_not be_hidden
    RubyShell::Entry.new("#{@sandbox_dir}/.dont_show").should be_hidden
  end

  it "is considered equal to entries with the same full path and on the same box" do
    same = RubyShell::Entry.new(@entry.full_path, @entry.box)
    @entry.should == same
  end

  it "is considered not equal to entries with the same full path on a different box" do
    same = RubyShell::Entry.new(@entry.full_path, RubyShell::Box.new('dummy'))
    @entry.should_not == same
  end

  it "can mimic another entry" do
    copy = RubyShell::Entry.new('abc', :dummy)
    copy.mimic(@entry)
